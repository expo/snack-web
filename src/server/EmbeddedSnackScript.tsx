...
const protocol = process.env.SNACK_SERVER_PROTOCOL || 'https';
const host = nullthrows(process.env.SNACK_SERVER_HOST);
...
(function() {
  ...
  var ExpoSnack = {
    append: function(container, options) {
      ...
      if (options.id) {
        iframe.src = '${protocol}://${host}/embedded/' + options.id + iframeQueryParams;
      } else {
        iframe.src = '${protocol}://${host}/embedded' + iframeQueryParams + '&waitForData=true';
      }
